...
# from graph_matching.matching import match_bcd_some_type, match_ot, match_triplet_ot
...
def main(opts):
    ...
    scene_graph_files.remove(opts.source_name + '.pkl')
    ...
    pred_s = torch.load(os.path.join(opts.scene_graph_folder, opts.source_name + '.pkl'))
    ...
    for file in tqdm(files_this_gpu):
        ...
        ranking_cost_this_gpu[file] = matching_method.match_triplet_hete_gw(
            graph_s, graph_t, 
            data_info['ind_to_classes'], data_info['ind_to_predicates'], obj_embedding, rel_embedding
        ).item()
    synchronize()
    ...
    if local_rank == 0:
        ...
        source_image = ImageOps.exif_transpose(Image.open(os.path.join(opts.image_folder, opts.source_name + '.jpg')))
        source_graph = torch.load(os.path.join(opts.scene_graph_folder, opts.source_name + '.pkl'))
        img_list.append(source_image.resize(size = (img_size, img_size)))
        graph_list.append(source_graph)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_name', type = str, default = '23')
    parser.add_argument('--image_folder', type = str, default = '/home/lzj/datasets/test')
    parser.add_argument('--scene_graph_folder', type = str, default = './outputs/test/out_scene_graphs')
    parser.add_argument('--result_folder', type = str, default = './outputs/test')
