...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0):
    ...
    for dataset in datasets:
        ...
        # the dataset information used for scene graph detection
        data_info = {}
        data_info['idx_to_files'] = dataset.custom_files if cfg.TEST.CUSTUM_EVAL else dataset.filenames
        data_info['ind_to_classes'] = dataset.ind_to_classes
        data_info['ind_to_predicates'] = dataset.ind_to_predicates

        if not os.path.exists(cfg.DETECTED_SGG_DIR):
            os.makedirs(cfg.DETECTED_SGG_DIR)

        with open(os.path.join(cfg.DETECTED_SGG_DIR, 'data_info.json'), 'w') as outfile:  
            json.dump(data_info, outfile)
        print('=====> ' + str(os.path.join(cfg.DETECTED_SGG_DIR, 'data_info.json')) + ' SAVED !')
